...
// Senden von "LED ein" bzw. "LED aus" an den ESP32 gespeichert in dem Array "value"
    ...
    func sendPattern(speed: Double, version: Int, loop: Bool, patternManager: PatternManager) {
        ...
        task = DispatchWorkItem { [weak self] in
            ...
            if peripheral.state == .connected {
                ...
                switch version {
                    ...
                    case 21: ledPattern = patternManager.fadePattern
    ...
    func sendWallpaper(circleColors: [Color]) {
            guard let peripheral = peripheral, let characteristic = characteristic else {
                print("Peripheral or characteristic not available. Waiting for connection...")
                return
            }
            if peripheral.state == .connected {
                currentTask?.cancel()
                let patternIdentifier: UInt8 = 6 // Für Wallpaper Farben
                    var value: [UInt8] = [patternIdentifier]
                    for color in circleColors {
                        value.append(UInt8(255 * color.rgbComponents.red))
                        value.append(UInt8(255 * color.rgbComponents.green))
                        value.append(UInt8(255 * color.rgbComponents.blue))
                    }
                    value.append(UInt8(255 * brightness))
                    let data = Data(value)
                    peripheral.writeValue(data, for: characteristic, type: .withResponse)
            }
        }
...
struct ContentView: View {
    ...
    var body: some View {
        TabView {
            ...
            HomeView()
                .tabItem {
                    tabItemLabel("Home", systemImage: "house")
                }
            // PatternView
            PatternView()
                .tabItem {
                    tabItemLabel("Pattern", systemImage: "paintpalette")
                }
            // MusicView
            MusicView()
                .tabItem {
                    tabItemLabel("Music", systemImage: "music.note")
                }
                .environmentObject(audioRecorderViewModel)
            // ShopView
            ShopView()
                .tabItem {
                    tabItemLabel("Shop", systemImage: "cart")
                }
            // SettingsView
            SettingsView()
                .tabItem {
                    tabItemLabel("Settings", systemImage: "gear")
                }
        }
        .background(
            Image("background1")
